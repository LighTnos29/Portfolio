...
- **Layout so far**: Global background with animated gradient "blobs", fixed glassmorphism navbar, techy glass hero section as the main above-the-fold content.
...
- **Navbar component**
  - Fixed at top (`fixed top-0 left-0 right-0 z-50`) over the animated background.
...
- **Hero component**
  - Full-width section below the navbar (`id="home"`) aligned with the dark + glassmorphism theme.
...
- **Primary text**
  ...
  - `text-white/50` – subtle section labels in the mobile menu.
...
- **`hero.jsx`**
  - Currently a placeholder; can be safely refactored into a richer hero section (headline, subtext, CTA, social links, etc.).
...
- **When refactoring**
  - If pulling styles into dedicated CSS/Tailwind config, keep this file as the source-of-truth description of the design language.
